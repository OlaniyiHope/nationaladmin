...
const EditPost = () => {
  ...
  // Add new videos, validated against how many existing ones are still kept
  const handleVideoChange = (e) => {
    const files = Array.from(e.target.files);
    const totalAfterAdd = existingVideos.length + files.length;

    if (totalAfterAdd > MAX_VIDEOS) {
      return toast.error(
        `You can have a maximum of ${MAX_VIDEOS} videos total. Remove an existing video first, or select fewer files.`
      );
    }
    if (files.some((f) => f.size > MAX_VIDEO_SIZE)) {
      return toast.error("Each video must be under 200MB.");
    }

    setPostVideos(files);
  };

  // Remove one specific existing (already-uploaded) video
  const handleRemoveExistingVideo = (videoUrl) => {
    setExistingVideos((prev) => prev.filter((v) => v !== videoUrl));
  };

  // Remove one specific newly-selected (not yet uploaded) video
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Keep whichever existing videos the user didn't remove
      if (existingVideos.length > 0) {
        formData.append("existingVideos", JSON.stringify(existingVideos));
      }

      // Upload any newly-added videos (these get merged with kept ones on the backend)
      if (postVideos.length > 0) {
        postVideos.forEach((video) => {
          formData.append("videos", video);
        });
      ...
      // Keep old thumbnail
      if (!videoImage && existingVideoImage) {
        formData.append("existingVideoImage", existingVideoImage);
      }
  ...
  return (
    <>
      ...
      <div className="p-6 max-w-2xl mx-auto">
        ...
        {/* ---------------- VIDEOS ---------------- */}
        <div className="mb-4">
          <label className="block font-medium mb-2">Existing Videos</label>

          {existingVideos.length > 0 ? (
            <div className="flex flex-wrap gap-3 mb-3">
              {existingVideos.map((video, idx) => (
                <div key={idx} className="relative">
                  <video src={video} controls className="w-64 border rounded" />
                  <button
                    type="button"
            ...
            <p className="text-gray-500 mb-3">No video found</p>
          )}

          <label className="block font-medium mb-1">
            Add / Replace Video{existingVideos.length < MAX_VIDEOS ? "s" : ""} (max {MAX_VIDEOS} total, 200MB each)
          </label>
          <input
            type="file"
            multiple
            accept="video/*"
            onChange={handleVideoChange}
            disabled={existingVideos.length >= MAX_VIDEOS}
            className="w-full"
          />
          {existingVideos.length >= MAX_VIDEOS && (
            <p className="text-xs text-gray-500 mt-1">
              You've reached the max of {MAX_VIDEOS} videos. Remove one above to add another.
            </p>
          )}

          {postVideos.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-3">
              {postVideos.map((video, idx) => (
                <div key={idx} className="relative">
                  <video src={URL.createObjectURL(video)} controls className="w-64 rounded border" />
                  <button
                    type="button"
        ...
        {/* ---------------- VIDEO THUMBNAIL ---------------- */}
        <div className="mb-6">
          <label className="block font-medium mb-1">Video Thumbnail Image</label>

          <input type="file" accept="image/*" onChange={handleVideoImageChange} />

          {existingVideoImage && !videoImage && (
            <img src={existingVideoImage} className="w-32 h-32 object-cover mt-2 rounded border" />
          )}

          {videoImage && (
            <img src={URL.createObjectURL(videoImage)} className="w-32 h-32 object-cover mt-2 rounded border" />
          )}
        </div>
